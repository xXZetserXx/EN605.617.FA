...
int main(int argc, char** argv) {
	...
	if (argc >= 2) {
			kernelFile = argv[1];
			std::cout << "========================================" << std::endl;
			std::cout << "Running kernel " << kernelMap[kernelFile] << " from " << kernelFile << ", " << std::endl;
			kernelCaller(kernelFile.c_str(), kernelMap[kernelFile].c_str());
	}
	else {
		for(auto& i : kernelMap) {
			std::cout << "========================================" << std::endl;
			std::cout << "Running kernel " << i.second << " from " << i.first << std::endl;
